...
jCarousel.set=function(arg){
    ...
    $(".jCarousel").each(function(){
        if(!$(this).attr("name")){
            ...
            jCarousel.sel[tNum]={width:tWidth,max:tMax,left:0,current:0,startX:0,endX:0,auto:0};
            ...
            $(".jCarouselWrapper"+tNum+' .jCarouselNavi a[rel="'+
              jCarousel.sel[tNum].current+'"]').addClass("selected");
            $(".jCarouselWrapper"+tNum).css({overflow:"hidden",width:"100%"});
            $(target).css({width:"900000px",listStyle:"none",padding:0,margin:0,backgroundColor:"transparent"});
            $("#jCarousel-object"+tNum+" > li").css("float","left");
            $("#jCarousel-object"+tNum+" > li").css({width:tWidth+"px",listStyle:"none",padding:0,margin:0,color:"#000"});
            $(".jCarouselWrapper"+tNum+" .jCarouselNavi").css({clear:"both",textAlign:"center"});
            $(".jCarouselWrapper"+tNum+" .jCarouselNavi a")
                .css({
            ...
            $(".jCarouselWrapper"+tNum+" .jCarouselNavi a.selected")
                .css({backgroundColor:jCarousel.color.active});
                ...
                $(window).bind("touchmove",function(){
                    if(jCarousel.activeBool){
                        ...
                        if(offsetX/offsetY>0.5||offsetX/offsetY<-0.5){
                            ...
                            $(jCarousel.target[tNum]).css({marginLeft:jCarousel.sel[tNum].left+offsetX+"px"});
...
jCarousel.slide=function(activeNum,carouselNum,speed){
    ...
    $(jCarousel.target[tNum]).animate({marginLeft:margin+"px"},speed,"easeCarousel",function(){});
    jCarousel.sel[tNum].left=margin;
    jCarousel.sel[tNum].current=carouselNum;
    $(".jCarouselWrapper"+tNum+" .jCarouselNavi a").css({backgroundColor:jCarousel.color.back});
    $(".jCarouselWrapper"+tNum+" .jCarouselNavi a[rel='"+carouselNum+"']")
        .css({backgroundColor:jCarousel.color.active});
};

jCarousel.next=function(activeNum,speed){
    jCarousel.slide(activeNum,(jCarousel.sel[activeNum].current + 1)%jCarousel.target[activeNum].childElementCount,speed);
};
jCarousel.prev=function(activeNum,speed){
    jCarousel.slide(activeNum,(jCarousel.sel[activeNum].current + jCarousel.target[activeNum].childElementCount - 1)%jCarousel.target[activeNum].childElementCount,speed);
};
...
jCarousel.resize=function(){
    $(".jCarousel").each(function(){
        ...
        jCarousel.sel[tNum].left=margin;
        $(target).find("li").css({width:tWidth+"px"});
        $(target).css({marginLeft:margin+"px"});
    });
};
jQuery.extend(
    jQuery.easing,{
